...
# Dom tree를 탐색하고 수정하는 기능을 제공하는 모듈로 웹 데이터 크롤링 또는 스크래핑을 할 때 사용.
...
# dom과 관련된 함수는 원노트 참고. 아래와 같이 find함수를 이용하면 전부 또는 인덱스 단위로 불러올 수 있다. (dom.a == dom.find("a")과 같음.)
...
# 이후 정규식을 활용해 $로 시작하는 부분만 긁어오는 방식 사용
...
# print(htmlDaumSearch.url)
# print(htmlDaumSearch.text)
