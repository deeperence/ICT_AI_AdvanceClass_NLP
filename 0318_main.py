...
path = "C:/windows/fonts/HMKMRHD.ttf" # 폰트를 지정하기 위한 경로 설정
font = font_manager.FontProperties(fname=path).get_name()
rc("font", family=font)


kcorpus = kolaw.open(kolaw.fileids()[0]).read()
ktokens = word_tokenize(kcorpus)
...
# print(txt, txt.vocab()) # <Text: Emma by Jane Austen 1816> <FreqDist with 8406 samples and 191785 outcomes> (각 토큰에 어떤 단어가 몇번씩 나왔는지 분포를 확인 가능.)
# print(txt, txt.vocab().most_common()) # 가장 많이 나온 요소들을 표시
# print(ktxt.vocab().most_common())
# txt.plot(50) # 빈도수를 plot으로 출력
# ktxt.plot(50) # 한글의 경우 폰트가 깨진다. matplot의 폰트 경로 설정을 해 주어야 함.
...
# 분포도를 통해 Jane은 주인공과 중간서부터 만나서 이야기를 끌어 나가는 역할을 하는 인물임을 알 수 있음. 이처럼 정확하지는 않아도 유의미한 피쳐를 유추할 수 있도록 도와준다.

# print(ktxt.count("국민")) # 국민의, 국민에 등등 형태소분석이 안되었으므로 2번밖에 나오지 않음.
...
# print(ntxt.count("소변을"))
# print(ntxt.concordance("소변을"))
...
def ngramUmheol(sentence, n=2): # 음절 단위로 구분하는 함수. sentence를 받아 2개씩 쪼갠다.
        result = []
        tokens_ngram = sentence.split()

        for i in range(len(tokens_ngram) - n+1):
            # result.append(tokens_ngram[i:i+n])
            # result.append(tuple(tokens_ngram[i:i+n])) # 튜플로 반환 시 키값을 쓸 수 있음
            result.append(" ".join(tokens_ngram[i:i+n]))
        return result

# print(ngramUmheol("국민을 위한", n=3)) # n 파라미터 개수에 따라 나뉘는 개수가 다름.
# print(ngramUmheol("국민을 위한 정부", n=2))

# ktokens_ngram = ngramUmheol(kcorpus, n=2)
# print(len(ktokens_ngram)) # 4177개.
# t = Text(ktokens_ngram)
...
3. 공백문자가 나오면 '_'로 치환
4. 각 단어의 끝에 단어의 마지막을 알려주는 </v> 태그 부착.
   ex) low -> {l o w </w> : 5}
5. 각 음절 분리
...
def makeTerm(sentence):
    terms = sentence.split() # 3번과정
    result = []

    for term in terms:
        result.append(" ".join(list(term) + ["</w>"])) # term을 list에 넣어 조각낸 후 공백을 삽입한 다음 문장의 끝을 나타내는 태그를 삽입

    return "_".join(result)

# print(makeTerm("low lower")) # makeTerm함수 테스트

def ngramSplit(data, n=2):
...
def mergeTokens(maxToken, data): # 9번과정 (많이 나온 두 게가 하나의 토큰으로 합쳐지게 된다.)
    ...
    자세히 보면 문자 앞뒤로 whitespace가 존재함을 알 수 있으며, whitespace로 시작하지 않아야 하며 끝나지도 않는 것을 찾아야 함.
...
for _ in range(5): # 이터레이션을 통해 빈도를 파악
    ...
    #         maxFreq = v
    #         maxToken = k
    # 하지만 우리는 dict에 저장을 해 두었으므로 max 함수를 사용하면 편하게 구현 가능(아래 세개 중 사용)
