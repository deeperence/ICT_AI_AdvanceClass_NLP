...
# browser.get(url)
# find_element는 하나만, find_elements는 다중 검색.
...
# result2 = browser.find_element_by_id("results")
# for tag in result2.find_elements_by_tag_name("a"):
#     print(tag.text)
#     print(tag.get_attribute("href"))

# print(len(browser.find_element_by_css_selector("#results a")))
...
# print(re.findall("<a href=\"(.+)\">(.+)</a>", result["records"][0]["pretty_link"])) # pattern이라는 정규식 패턴과 원본 문자열 입력)

browser.get("https://nid.naver.com/nidlogin.login")
accountInfo = {"id":"aaaaaa", "pw":"bbbbbb"} # ID, PW는 개인 ID와 PW 입력.

# find_element_by_name과 같이 함수로만 쓰게 되면 셀레니움을 안쓰거나 브라우져가 바뀌는 순간 사용이 어렵게 됨. 따라서 CSS나 xpath를 사용하는 방법을 알아야 함.
# browser.find_element_by_css_selector("#id).clear() -> CSS를 사용하는 경우
# browser.find_element_by_xpath("//input[@name='id']").clear() -> XPath를 사용하는 경우
# browser.find_element_by_name("id").clear() -> 그냥 함수만 사용하는 경우
...
# 쿠키값을 그대로 전달할 수 있다면 로그인 정보를 넘길 수 있을 것이고, 후속 url들을 원활히 탐색할 수 있을 것.
...
# 자바스크립트가 가득한 사이트는 정보를 받아오는데에 일정 시간이 걸리며, 따라서 눈에 보이지 않는 기간 동안, 또는 특정 이벤트를 통해서만 발동되는 경우 클릭을 할 수 없기 때문.
# 따라서 셀레니움은 두 가지 기능 제공 : 1)단순히 sleep 거는 것, 2)특정한 무언가가 모습을 나타날 때까지 pending. (sleep을 걸면 모든 사이트가 멈추기 때문에 비동기적으로 작업하기 위해선 안쓰는것이 좋음.)
# browser.find_element_by_css_selector(".gnb_btn_login").click() # not visible exception에러 발생. 하지만 .get_attribute()를 붙여 받아보면 존재하는 것을 확인 가능


# 특정 태그가 나타날때까지 비동기적으로 기다릴 수 있게 해줌. 드라이버(browser)를 받아 1초동안 10번 확인하되 ElementNotVisibleException가 발생할 시엔 그냥 넘어간다.
browserWait = WebDriverWait(browser, 10, 1, ElementNotVisibleException) # wait를 위한 인스턴스 생성

# 우리가 전달한 browser를 x라고 했을때, is_displayed가 참/거짓인지를 검사. 구문 실행중 10초 이내에 프로필 영역을 클릭해서 로그아웃 버튼이 렌더링된 경우 true를 반환.
