...
# 네이트에서 'python' 사이트 검색결과의 제목 가져오기
htmlNateSearch = getDownload("https://search.daum.net/nate?thr=sbma&w=tot", {"q":"python"})
domNateSearch = BeautifulSoup(htmlNateSearch.text, "html.parser")
# nodeNateSearch = domNateSearch.find_all("", {"class" : "f_link_b"}) # 테스트를 위한 임시 print문

# 사이트 검색 부분의 유일한 key-value쌍인 "disp-attr":"IVR"으로 find한 후 find_all을 통해 해당 트리 아래에 있는 모든 "class":"wrap_tit"를 가져옴.
for tag in domNateSearch.find("", {"disp-attr":"IVR"}).find_all("", {"class":"wrap_tit"}):
    print(tag.text)
    print(tag.find("a")["href"])