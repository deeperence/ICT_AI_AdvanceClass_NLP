...
# question에 대한 answer를 가져옴.
# robot = robotparser.RobotFileParser()
# robot.set_url('https://google.com/robots.txt')
# robot.read() #리퀘스트를 날림.
# print(robot.can_fetch('Agent', 'https://google.com/robots.txt')) #'내 봇의 이름이 이런데, 저 url에 대해 크롤링해도 되는가?'

# robot = robotparser.RobotFileParser() #인스턴스 생성
# robot.set_url('https://www.koipa.or.kr/robots.txt') #가져올 사이트 정의
# robot.read() #리퀘스트를 날림.
# print(robot.can_fetch('Agent', 'https://www.koipa.or.kr/robots.txt')) #'내 봇의 이름이 이런데, 이 url에 대해 크롤링해도 되는가?'
...
# print(resp.getcode()) # 다른페이지에서 응답을 받은 상태.(원래 접근하지 말아야 할 페이지에 접근하고자 했기때문에, 대신 다른 페이지의 정보를 발신.) 200이 떴으므로 정상이다.
...
# bot은 user-agent가 None이라 권한이 없음. 따라서 헤더에 user-agent정보를 넣어서 속여야 함.
# (크롬 F12 개발자도구->Network 열어두고 구글검색 한번 하면 TCP/IP 주고받는 내역이 뜸. 그 중에서 맨 첫번째 ?search부분에 있음.)
...
# html = download("https://www.google.com/search?q=python")
# html.read().decode("utp-8")
...
# 이를 통해 사이트를 옮겨다니면서 앵커 태그를 타고 네트워크를 돌아다닐 수 있게 됨. (https://www.google.com/search/about)
...
resp.content # 알아서 바이트로 받아옴. (바이트타입으로 변환된 결과물을 들고 있으므로 굳이 변환 안해줘도 됨.)
html = download2("get",
